 ...
 *@brief  Header Class for PositiveSentiment that analyzes text for Negative sentiment.
...
class PositiveSentiment {
 public:
 /**
  * @brief analysis for PositiveSentiment calculates the negative score for input.
  * @return std::string.
 */
std::string analysis(std::map<std::string, int> histogram);

 /**
  * @brief loadwordlist for the PositiveSentiment class loads a negative wordlist.
 */
void loadWordlist();
